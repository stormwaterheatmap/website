...
The stormwater outfall data is available from Ecology via an open-data api at: [https://data.wa.gov/Natural-Resources-Environment/Municipal-Stormwater- Permit-Outfall-Data/d958-q2ci.](https://data.wa.gov/Natural-Resources-Environment/Municipal-Stormwater-%20Permit-Outfall-Data/d958-q2ci.%20)
...
Table 1 Percentage of data points that were left-censored (non-detect) for each chemical of concern
...
Figure 2 Quantile-quantile plots of COCs using a natural-log (ln) scale. Red line shows the QQ-line.
...
Figure 3 Strong predictors for copper, showing linear model fit (blue line) for the relationship between ln-transformed copper concentration and each predictor in turn.
...
Figure 4 Normalized residuals from beyond-optimal model, with no variance structure (left), and with the best fit variance structure (right).
...
$ln(copper) ~ rain + summer + traffic + devAge2 + pm25\_na\ +\ \left(rain\ \times pm25\_na\right)$
...
Figure 5 Single-predictor plots for copper, fit of the Landscape Predictor Model to each predictor in turn.

<img src="media\image8.png" style="width:6in;height:4.00213in"/>

Figure 6 Plot showing the interaction between rain and pm25_na that is present in the best fit model. In areas with high pm25_na values, increasing amounts of rain result in a dilution of copper in stormwater
...
Figure 7 Copper model residuals for the Null Model, Categorical Land Use Model, and Landscape Predictor Models. Each bar represents one watershed, with colors representing agencies.

Table 2 Coefficient values (standard error in parenthesis) for the three copper models. For the Categorical Landuse Model, the baseline landuse is LDR; all other land use categories are adjustments from the baseline. Final coefficient values for linear mix
...
Figure 8 Model coefficients for the Null Model (green), Categorical Land Use Model (blue), and Landscape Predictor Model (red).
...
$$
var(\epsilon_{ijk}) = \sigma^2_je^{2\delta(rain_{ik})}
$$

<img src="media\image13.png" style="width:5.99927in;height:2.3in"/>

Figure 10 Normalized residuals from beyond-optimal model, with no variance structure (left), and with the best fit variance structure (right).
...
Figure 11 TSS model residuals for the Null Model, Categorical Land Use Model, and Landscape Predictor Models. Each bar represents one watershed, with colors representing agencies.

Table 3 Coefficient values (standard error in parenthesis) for the three TSS models. For the Categorical Landuse Model, the baseline landuse is LDR; all other land use categories are adjustments from the baseline. Final coefficient values for linear mixed effects

<img src="media\image16.png" style="width:6in;height:2.06313in"/>

<img src="media\image17.png" style="width:6in;height:4.5in"/>

Figure 12 Model coefficients for the Null Model (green), Categorical Land Use Model (blue), and Landscape Predictor Model (red).
...
Figure 14 Normalized residuals from beyond-optimal model, with no variance structure (left), and with the best fit variance structure (right).
...
$$
\ln(phosphorus) \sim rain + summer + grass +paved + \sqrt{(CO^2_{road}}$$
...
Figure 15 Single-predictor plots for phosphorus, showing fit of the Landscape Predictor Model to each predictor.
...
Figure 16 Phosphorus model residuals for the Null Model, Categorical Land Use Model, and Landscape Predictor Models. Each bar represents one watershed, with colors representing agencies.
...
Table 5 Model coefficients for the Null Model (green), Categorical Land Use Model (blue), and Landscape Predictor Model (red).
...
[2] See: [~~https://fortress.wa.gov/ecy/gispublic/DataDownload/ECY_CAD_Landuse2010~~](https://fortress.wa.gov/ecy/gispublic/DataDownload/ECY_CAD_Landuse2010){.uri}~~. htm~~ for more information \*\*\* NOTE \*\*\* this needs to be updated for commerce data!
